...
2. Double-click Setup and start the installation. Setup first validates the embedded MSIX and public certificate while retaining the identity of the Windows user who started it. During MSIX deployment, the progress bar displays the actual percentage reported by Windows.
3. On the first installation, Windows requests administrator approval if the certificate is not trusted yet. The elevated helper only imports the public RightAgent certificate into Local Computer\Trusted People, never Trusted Root Certification Authorities. After the helper exits, the original user process installs the MSIX for the current user. Cancelling User Account Control prevents MSIX installation. Upgrades do not request elevation again while the certificate remains trusted.
