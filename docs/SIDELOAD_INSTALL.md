...
2. 双击安装器并开始安装。安装器会先验证内嵌 MSIX 与公共证书，并始终保留最初发起安装的 Windows 用户身份；进入 MSIX 部署阶段后，进度条会显示 Windows 返回的真实安装百分比。
3. 首次安装若证书尚未受信任，Windows 会请求一次管理员批准。管理员辅助进程只把公共 RightAgent 证书导入“本地计算机\受信任的人”，不会导入“受信任的根证书颁发机构”；辅助进程退出后，原用户进程继续为当前用户安装 MSIX。取消用户账户控制提示后不会安装 MSIX。证书已经受信任的升级不会重复请求管理员权限。
